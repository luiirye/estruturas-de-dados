...
int Banco_vazio(Banco *CABECA) {
    if (CABECA->proximo == NULL) {
        ...
        return 1; // Verdade, lista está vazia.
    } else {
        printf("O Banco possui clientes.\n");
        return 0; // Falso, a lista Banco não está vazia.
    }
}
...
Banco *insere_ordenado(Banco *CABECA) {
    ...
    novo->numero_conta = x;
    strcpy(novo->nome_cliente, nome); // Copia o nome do cliente
    novo->saldo = valor;
    novo->proximo = NULL;
...
Banco *remove_inicio(Banco *CABECA, int x) {
    ...
    Banco *aux = CABECA->proximo; // Ponteiro para o primeiro cliente
    ...
    p->proximo = aux->proximo; // Remove o cliente da lista
    free(aux); // Libera a memória do cliente removido
    return CABECA;
}
